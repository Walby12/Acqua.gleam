...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(QS)) -> {ok, QS} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(QM)) -> {ok, QM} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(QI)) -> {ok, QI} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(QY), fun((QY) -> json())) -> json().
...
-spec array(list(RC), fun((RC) -> json())) -> json().
...
-spec dict(gleam@dict:dict(RG, RH), fun((RG) -> binary()), fun((RH) -> json())) -> json().
