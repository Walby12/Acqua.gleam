...
-spec map_first({CPB, CPC}, fun((CPB) -> CPD)) -> {CPD, CPC}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CPE, CPF}, fun((CPF) -> CPG)) -> {CPE, CPG}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
