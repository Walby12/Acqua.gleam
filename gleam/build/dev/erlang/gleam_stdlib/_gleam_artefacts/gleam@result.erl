...
-spec map({ok, CPS} | {error, CPT}, fun((CPS) -> CPW)) -> {ok, CPW} |
    {error, CPT}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CPZ} | {error, CQA}, fun((CQA) -> CQD)) -> {ok, CPZ} |
    {error, CQD}.
...
-spec flatten({ok, {ok, CQG} | {error, CQH}} | {error, CQH}) -> {ok, CQG} |
    {error, CQH}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CQO} | {error, CQP}, fun((CQO) -> {ok, CQS} | {error, CQP})) -> {ok,
        CQS} |
    {error, CQP}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CQX} | {error, CQY}, fun((CQX) -> {ok, CRB} | {error, CQY})) -> {ok,
        CRB} |
    {error, CQY}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CRG} | {error, any()}, CRG) -> CRG.
...
-spec lazy_unwrap({ok, CRK} | {error, any()}, fun(() -> CRK)) -> CRK.
...
-spec unwrap_error({ok, any()} | {error, CRP}, CRP) -> CRP.
...
-spec unwrap_both({ok, CRS} | {error, CRS}) -> CRS.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CRV} | {error, CRW}, {ok, CRV} | {error, CRW}) -> {ok, CRV} |
    {error, CRW}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CSD} | {error, CSE}, fun(() -> {ok, CSD} | {error, CSE})) -> {ok,
        CSD} |
    {error, CSE}.
...
-spec all(list({ok, CSL} | {error, CSM})) -> {ok, list(CSL)} | {error, CSM}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 368).
-spec partition_loop(list({ok, CTA} | {error, CTB}), list(CTA), list(CTB)) -> {list(CTA),
    list(CTB)}.
...
-spec partition(list({ok, CST} | {error, CSU})) -> {list(CST), list(CSU)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CTJ}, CTM) -> {ok, CTM} | {error, CTJ}.
...
-spec replace_error({ok, CTP} | {error, any()}, CTT) -> {ok, CTP} | {error, CTT}.
...
-spec values(list({ok, CTW} | {error, any()})) -> list(CTW).
values(Results) ->
