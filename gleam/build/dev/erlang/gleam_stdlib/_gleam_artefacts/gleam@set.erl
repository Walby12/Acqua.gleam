...
-opaque set(CZB) :: {set, gleam@dict:dict(CZB, list(nil))}.
...
-spec contains(set(CZM), CZM) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZO), CZO) -> set(CZO).
delete(Set, Member) ->
...
-spec fold(set(CZX), CZZ, fun((CZZ, CZX) -> CZZ)) -> CZZ.
...
-spec filter(set(DAA), fun((DAA) -> boolean())) -> set(DAA).
...
-spec drop(set(DAH), list(DAH)) -> set(DAH).
drop(Set, Disallowed) ->
...
-spec take(set(DAL), list(DAL)) -> set(DAL).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DAT), set(DAT)) -> {set(DAT), set(DAT)}.
order(First, Second) ->
...
-spec intersection(set(DAY), set(DAY)) -> set(DAY).
...
-spec difference(set(DBC), set(DBC)) -> set(DBC).
...
-spec is_subset(set(DBG), set(DBG)) -> boolean().
...
-spec is_disjoint(set(DBJ), set(DBJ)) -> boolean().
...
-spec each(set(DBQ), fun((DBQ) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CZJ), CZJ) -> set(CZJ).
insert(Set, Member) ->
...
-spec from_list(list(CZU)) -> set(CZU).
from_list(Members) ->
...
-spec map(set(DAD), fun((DAD) -> DAF)) -> set(DAF).
map(Set, Fun) ->
...
-spec union(set(DAP), set(DAP)) -> set(DAP).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DBM), set(DBM)) -> set(DBM).
